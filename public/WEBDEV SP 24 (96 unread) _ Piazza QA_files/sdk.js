...
(function(window){
    ...
    var SEC = function() {
        ...
        this.create = function(index, element) {
            ...
            if (element.getAttribute('data-hide-load-button') != null) {
                var hide = element.getAttribute('data-hide-load-button');
                hide = hide.toString().toLowerCase();
                if (hide == '1' || hide == 'true') {
                    iframe.src = iframe.src + '&hide_load_button=1';
                }
            }
    ...
    SECWidgetEvents.events = [
        ...
        'languageChanged', 'sourceChanged', 'inputChanged',
    ];
    ...
    */
                    ...
                    if (widget !== null) {
                        switch (event.data.event) {
                            ...
                            case 'beforeSendSubmission':
                                var output = widget.events.execute('beforeSendSubmission', event.data.data);
                                var message = {event: 'beforeSendSubmission'}

                                if (typeof output == 'object' && output !== null) {
                                    message['result'] = true;
                                    if (output.hasOwnProperty('submissionSource')) {
                                        message['submissionSource'] = output['submissionSource'];
                                    }
                                    if (output.hasOwnProperty('submissionLanguage')) {
                                        message['submissionLanguage'] = output['submissionLanguage'];
                                    }
                                    if (output.hasOwnProperty('submissionInput')) {
                                        message['submissionInput'] = output['submissionInput'];
                                    }
                                    if (output.hasOwnProperty('customData')) {
                                        message['customData'] = output['customData'];
                                    }
                                } else {
                                    message['result'] = !!output;
                                }
                                widget.iframe.contentWindow.postMessage(message, '*');
                                break;
                            ...
                            case 'languageChanged':
                                widget.events.execute('languageChanged', event.data.data);
                                break;
                            case 'sourceChanged':
                                widget.events.execute('sourceChanged', event.data.data);
                                break;
                            case 'inputChanged':
                                widget.events.execute('inputChanged', event.data.data);
                                break;
                        }
                    }
                }
    ...
    /**
     * Destroy widget.
     * Removes all widget data.
     *
     * @param widget
     */
    SEC.prototype.destroyWidget = function(widget) {

        if (typeof widget !== 'object' || typeof widget.iframe !== 'object') {
            SECUtils.log('error', null, 'invalid widget object');
            return;
        }
     ...
     * Destroy widget.
     *
     * Remove the IFRAME and clean widget data.
     */
    SECWidget.prototype.destroy = function () {

        if (window.SEC) {
            window.SEC.destroyWidget(this);
        }

        SECUtils.log('info', 'widget', 'destroy', window.SEC.debug);
    };
     ...
     * @param {String} codeInput input data
     */
    SECWidget.prototype.setInput = function (codeInput) {

        var _widget = this;
        this.ready(function() {
            _widget.iframe.contentWindow.postMessage({
                event: 'setInput',
                data: typeof codeInput !== 'undefined' ? codeInput : null
            }, '*');

            SECUtils.log('info', 'widget', 'set input', window.SEC.debug);
        });
    };
     ...
     * @param {String} codeSource source code
     */
    SECWidget.prototype.setSource = function (codeSource) {

        var _widget = this;
        this.ready(function() {
            _widget.iframe.contentWindow.postMessage({
                event: 'setSource',
                data: typeof codeSource !== 'undefined' ? codeSource : null
            }, '*');

            SECUtils.log('info', 'widget', 'set source', window.SEC.debug);
        });
    };
     ...
     * @param {Number} codeCompiler the ID of compiler
     */
    SECWidget.prototype.setCompiler = function (codeCompiler) {

        var _widget = this;
        this.ready(function() {
            _widget.iframe.contentWindow.postMessage({
                event: 'setCompiler',
                data: typeof codeCompiler !== 'undefined' ? codeCompiler : null
            }, '*');

            SECUtils.log('info', 'widget', 'set compiler', window.SEC.debug);
        });
    };

    SECWidget.prototype.enableRun = function () {
        var _widget = this;
        this.ready(function() {
            _widget.iframe.contentWindow.postMessage({event: 'enableRun'}, '*');
            SECUtils.log('info', 'widget', 'enable run', window.SEC.debug);
        });
    }

    SECWidget.prototype.disableRun = function () {
        var _widget = this;
        this.ready(function() {
            _widget.iframe.contentWindow.postMessage({event: 'disableRun'}, '*');
            SECUtils.log('info', 'widget', 'disable run', window.SEC.debug);
        });
    }
